...
export default function CalibrationPage() {
  ...
  return (
    <div className="bg-[#07111F] min-h-screen text-white w-full max-w-none px-6 md:px-12 py-8">
      ...
      {/* Main Calibration Optimization Table View */}
      <div className="bg-[#121B2B] border border-[#1F2937] rounded-xl p-5 md:p-8">
        ...
        {calculatedRows.length === 0 ? (
          ...
          <div className="w-full overflow-x-auto">
            <div className="w-full min-w-[800px]">
              ...
              {/* Main Column Headers */}
              <div className="grid grid-cols-[4fr_4fr_4fr] gap-4 px-4 py-3 border-b border-[#1F2937] text-xs font-bold uppercase tracking-[2px] text-gray-400 items-center">
                <div>Parameter</div>
              ...
              {/* Multi-Row list per Parameter */}
              {calculatedRows.map(([key, value], index) => {
                const normalizedKey = normalizeKey(key);
                const unit = value?.unit || getUnit(key);

                const currentBaseline = baselineSnapshot[normalizedKey] || "0.00";
                const currentLimits = baselineRanges[normalizedKey];
                const calculatedInputValue = latestParams[normalizedKey] || "0.00";

                return (
                  <div key={index} className="grid grid-cols-[4fr_4fr_4fr] gap-4 px-4 py-5 border-b border-[#182232] items-center hover:bg-[#0D1625] transition-all">
                    
                    {/* Column 1: Title */}
                    <div>
                      <div className="text-white text-base font-bold break-words">{key}</div>
                      {unit && <span className="text-xs text-gray-500 mt-1 block">Unit: {unit}</span>}
                    </div>

                    {/* Column 2: Current Limits Sub-Rows */}
                    <div className="space-y-2 text-sm border-r border-gray-800/60 pr-2">
                      <div className="flex justify-between px-2 py-0.5">
                        <span className="text-gray-500 text-xs">Baseline:</span>
                        <span className="text-cyan-400 font-semibold">{currentBaseline} {unit}</span>
                      </div>
                      <div className="flex justify-between px-2 py-0.5">
                        <span className="text-gray-500 text-xs">Lower Limit:</span>
                        <span className="text-green-400 font-medium">
                          {currentLimits ? currentLimits.lower_tolerance.toFixed(2) : "0.00"} {unit}
                        </span>
                      </div>
                      <div className="flex justify-between px-2 py-0.5">
                        <span className="text-gray-500 text-xs">Upper Limit:</span>
                        <span className="text-red-400 font-medium">
                          {currentLimits ? currentLimits.upper_tolerance.toFixed(2) : "0.00"} {unit}
                        </span>
                      </div>
                    </div>

                    {/* Column 3: Calculated Limits Sub-Rows + Interactive Set Input */}
                    <div className="space-y-2 text-sm pl-2">
                      <div className="flex justify-between items-center px-2">
                        <span className="text-gray-500 text-xs">New Target:</span>
                        <div className="relative w-32">
                          <input
                            ...
                            className="w-full bg-[#07111F] border border-[#1F2937] rounded-md px-2 py-1 text-right text-xs text-white outline-none focus:border-cyan-400 font-bold"
                          />
                        </div>
                      </div>
                      <div className="flex justify-between px-2 py-0.5">
        ...
        <div className="flex flex-col sm:flex-row justify-end items-center gap-4 mt-8 md:mt-10">
          ...
          <button
            ...
            className={`
              font-bold px-8 py-3.5 rounded-xl text-base tracking-wide uppercase transition-colors
              ${isApplying || calculatedRows.length === 0
                  ? "bg-cyan-600/30 text-white/20 cursor-not-allowed"
                  : "bg-cyan-500 hover:bg-cyan-400 text-black shadow-[0_0_15px_rgba(34,211,238,0.2)]"
              }
            `}
